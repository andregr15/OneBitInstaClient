...
export class PostFormatter {
  ...
  private includeUser(user_data: any, included_data: any): User {
    let user = included_data.filter(
      included => included.type == "user" && included.id == user_data.id
    )[0];
  ...
  private includeHashtags(post_hashtags: any, included_data: any): Hashtag {
    let hashtags_id = post_hashtags.map(hashtag => hashtag.id);
    return included_data.filter(
      included => included.type == "hashtag" && this.isThereHashtagId(hashtags_id, included.id)
    ).map(
      hashtag => new Hashtag(hashtag.id, hashtag.attributes.name)
    );
  }
